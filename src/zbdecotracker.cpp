...
CzBDECOTracker::CzBDECOTracker(std::string strWalletFile)
{
...
CzBDECOTracker::~CzBDECOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBDECOTracker::Init()
{
...
bool CzBDECOTracker::Archive(CMintMeta& meta)
{
...
bool CzBDECOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBDECOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBDECOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBDECOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBDECOTracker::GetSerialHashes()
{
...
CAmount CzBDECOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBDECOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBDECOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBDECOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBDECOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBDECOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBDECOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBDECOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBDECOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBDECOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBDECOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBDECOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBDECOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBDECOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBDECOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBDECOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBDECOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzbdeco from DB\n", __func__, listDeterministicDB.size());
    }
