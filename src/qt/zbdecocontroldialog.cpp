...
// Copyright (c) 2017-2018 The BDCashProtocol developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zbdecocontroldialog.h"
#include "ui_zbdecocontroldialog.h"
...
std::set<std::string> ZBDecoControlDialog::setSelectedMints;
std::set<CMintMeta> ZBDecoControlDialog::setMints;

ZBDecoControlDialog::ZBDecoControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBDecoControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBDecoControlDialog::~ZBDecoControlDialog()
{
    delete ui;
}

void ZBDecoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBDecoControlDialog::updateList()
{
...
void ZBDecoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBDecoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBDeco_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBDecoControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBDecoControlDialog::GetSelectedMints()
{
...
void ZBDecoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
