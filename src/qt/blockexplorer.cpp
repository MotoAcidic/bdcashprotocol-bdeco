...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::BDECO, nValue);
...
{
    std::string genesisblockhash = "0000077da25fcf1369e4e6428abb2e082663a0f2d769d31f393190d6922f8634";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (bdcashprotocol.conf).");
            QMessageBox::warning(this, "BDCashProtocol Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
