...
bool Intro::pickDataDirectory()
{
    ...
    dataDir = settings.value("strDataDirNew", dataDir).toString();
    ...
    if (!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || GetBoolArg("-choosedatadir", false)) {
        ...
        while (true) {
            ...
            try {
                ...
                QMessageBox::critical(0, tr("BDCashProtocol"),
                    tr("Error: Specified data directory \"%1\" cannot be created.").arg(dataDir));
                /* fall through, back to choosing screen */
            }
        }

        settings.setValue("strDataDirNew", dataDir);
    }
    /* Only override -datadir if different from the default, to make it possible to
     * override -datadir in the bdcashprotocol.conf file in the default data directory
     * (to be consistent with bdcashprotocold behavior)
     */
    if (dataDir != getDefaultDataDirectory())
