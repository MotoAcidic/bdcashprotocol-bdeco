...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BDECO);
    ...
    if (!settings.contains("nAnonymizeBDCashProtocolAmount"))
        settings.setValue("nAnonymizeBDCashProtocolAmount", 1000);

    nAnonymizeBDCashProtocolAmount = settings.value("nAnonymizeBDCashProtocolAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBDCashProtocolAmount"))
        SoftSetArg("-anonymizebdcashprotocolamount", settings.value("nAnonymizeBDCashProtocolAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bdcashprotocol.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBDCashProtocolAmount:
            return QVariant(nAnonymizeBDCashProtocolAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBDCashProtocolAmount:
            nAnonymizeBDCashProtocolAmount = value.toInt();
            settings.setValue("nAnonymizeBDCashProtocolAmount", nAnonymizeBDCashProtocolAmount);
            emit anonymizeBDCashProtocolAmountChanged(nAnonymizeBDCashProtocolAmount);
            break;
        case CoinControlFeatures:
