...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered bdcashprotocol address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BDECO.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal bdcashprotocol:URI (bdcashprotocol:XyZ...?message=example)
            {
...
static void NotifyzBDECOReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBDECOReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBDECOReset.connect(boost::bind(NotifyzBDECOReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBDECOReset.disconnect(boost::bind(NotifyzBDECOReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbdecoTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
