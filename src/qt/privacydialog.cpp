                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBDECO ought to be enough for anybody." - Bill Gates, 2017
    ui->zBDECOpayAmount->setValidator( new QDoubleValidator(0.0, 14190000.0, 20, this) );
    ...
    ui->labelzBDECOSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBDECOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBDECO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBDECO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zBDECO, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBDECO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBDECO in ") +
...
void PrivacyDialog::on_pushButtonSpendzBDECO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBDECO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zBDECO, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zBDECO
        sendzBDECO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBDECO
    sendzBDECO();
}

void PrivacyDialog::on_pushButtonZBDecoControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZBDecoControlDialog* zBDecoControl = new ZBDecoControlDialog(this);
    zBDecoControl->setModel(walletModel);
    zBDecoControl->exec();
}

void PrivacyDialog::setZBDecoControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBDecoSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBDECO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BDCashProtocol Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBDECOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBDECO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BDECO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBDECO</b>";
    ...
    // use mints from zBDECO selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBDecoControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZBDecoControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zBDECO require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zBDECO"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZBDECO_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zBDECO require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zBDECO"));
            ui->TEMintStatus->repaint();
            return;
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zBDECO transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zBDECO was spent successfully update the addressbook with the label
    ...
    // Clear zbdeco selector in case it was used
    ZBDecoControlDialog::setSelectedMints.clear();
    ui->labelzBDecoSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBDECO Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zbdecoTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBDECO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBDECO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBDECO "));
    ui->labelzBDECOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zBDECO </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zBDECO </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzBDECOSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzBDECO->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zBDECO
        ui->pushButtonMintzBDECO->setEnabled(false);
        ui->pushButtonMintzBDECO->setToolTip(tr("zBDECO is currently disabled due to maintenance."));

        // Spend zBDECO
        ui->pushButtonSpendzBDECO->setEnabled(false);
        ui->pushButtonSpendzBDECO->setToolTip(tr("zBDECO is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zBDECO
        ui->pushButtonMintzBDECO->setEnabled(true);
        ui->pushButtonMintzBDECO->setToolTip(tr("PrivacyDialog", "Enter an amount of BDECO to convert to zBDECO", 0));

        // Spend zBDECO
        ui->pushButtonSpendzBDECO->setEnabled(true);
        ui->pushButtonSpendzBDECO->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
