...
CzBDECOWallet::CzBDECOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbdeco seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBDECOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBDECOSeed_deprecated()) {
                    LogPrintf("%s: Updated zBDECOseed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbdeco seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbdeco wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBDECOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBDECOCount(nCountLastUsed);
    else if (!walletdb.ReadZBDECOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBDECOWallet::Lock()
{
    seedMaster = 0;
}

void CzBDECOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBDECOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBDECO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBDECOWallet::LoadMintPoolFromDB()
{
...
void CzBDECOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBDECOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBDECOWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbdecoTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBDECOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBDECO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbdecoTracker which also adds to database
    pwalletMain->zbdecoTracker->Add(dMint, true);
...
void CzBDECOWallet::SeedToZBDECO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBDECOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBDECOWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZBDECOCount(nCountLastUsed);
...
void CzBDECOWallet::GenerateDeterministicZBDECO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBDECOWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBDECOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
