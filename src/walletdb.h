...
public:
    ...
    bool WriteZBDECOSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBDECOSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBDECOSeed_deprecated(uint256& seed);
    bool EraseZBDECOSeed();
    bool EraseZBDECOSeed_deprecated();

    bool WriteZBDECOCount(const uint32_t& nCount);
    bool ReadZBDECOCount(uint32_t& nCount);
