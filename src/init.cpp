...
#include "config/bdcashprotocol-config.h"
#endif
...
CzBDECOWallet* zwalletMain = NULL;
...
void PrepareShutdown()
{
    ...
    RenameThread("bdcashprotocol-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "bdcashprotocol.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "bdcashprotocold.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BDECOand zBDECOmoney supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 17293, 27293));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in BDECO/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in BDECO/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, bdcashprotocol, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in BDECO/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all BDCashProtocol specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zcrstake=<n>", strprintf(_("Enable or disable staking functionality for BDECOinputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbdecostake=<n>", strprintf(_("Enable or disable staking functionality for zBDECOinputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:17293"));
    ...
    strUsage += HelpMessageOpt("-backupzbdeco=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBDECOminting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbdecobackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zBDECObackup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizebdcashprotocolamount=<n>     " + strprintf(_("Keep N BDECOanonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The BDCashProtocol Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("bdcashprotocol-loadblk");
...
/** Sanity checks
 *  Ensure that BDCashProtocol is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. BDCashProtocol is shutting down."));
...
#endif
    // Make sure only a single BDCashProtocol process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BDCashProtocol is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("BDCashProtocol version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //BDCashProtocol: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //BDCashProtocol specific: zerocoin and spork DB's
                ...
                // BDCashProtocol: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZBDECOMinted();
                        RecalculateZBDECOSpent();
                    }
                    RecalculateBDECOSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // BDCashProtocol: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BDCashProtocol") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart BDCashProtocol to complete") << "\n";
        ...
        zwalletMain = new CzBDECOWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zBDECOWallet
        uiInterface.InitMessage(_("Syncing zBDECOwallet..."));

        bool fEnableZBDecoBackups = GetBoolArg("-backupzbdeco", true);
        pwalletMain->setZBDecoAutoBackups(fEnableZBDecoBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zbdecoTracker->Init();
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeBDCashProtocolAmount = 2;
...
//    }
//
//    nAnonymizeBDCashProtocolAmount = GetArg("-anonymizebdcashprotocolamount", 0);
//    if (nAnonymizeBDCashProtocolAmount > 999999) nAnonymizeBDCashProtocolAmount = 999999;
//    if (nAnonymizeBDCashProtocolAmount < 2) nAnonymizeBDCashProtocolAmount = 2;
    ...
    LogPrintf("Anonymize BDCashProtocol Amount %d\n", nAnonymizeBDCashProtocolAmount);
    ...
    /* Denominations
       ...
       For example:
       1BDECO+1000 == (.1BDECO+100)*10
       10BDECO+10000 == (1BDECO+1000)*10
    */
